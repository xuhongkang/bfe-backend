...
const utils_1 = require("../schema-model/parser/utils");
...
function getObjFieldMeta({ obj, objects, interfaces, scalars, unions, enums, callbacks, customResolvers, validateResolvers, }) {
    ...
    return obj?.fields?.reduce((res, field) => {
        ...
        if (jwtClaimDirective) {
            if (!objIsJwtPayload) {
                throw new Error("@jwtClaim directive can only be used on fields within a type annotated with @jwtPayload");
            }
            if (field.directives.length > 1) {
...
function isListValue(value) {
    return value.kind === graphql_1.Kind.LIST;
}
...
function parseSelectableDirective(directive) {
    ...
    const args = directive ? (0, utils_1.parseArguments)(directive) : {};
    return {
