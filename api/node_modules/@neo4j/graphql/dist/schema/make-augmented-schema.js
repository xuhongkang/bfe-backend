...
const create_relationship_fields_1 = __importDefault(require("./create-relationship-fields/create-relationship-fields"));
...
function makeAugmentedSchema(document, { features, enableRegex, validateResolvers, generateSubscriptions, callbacks, userCustomResolvers, subgraph, } = { validateResolvers: true }) {
    const composer = new graphql_compose_1.SchemaComposer();
    let relationships = [];
    const createInfo = composer.createObjectTC(CreateInfo_1.CreateInfo);
    const deleteInfo = composer.createObjectTC(DeleteInfo_1.DeleteInfo);
    const updateInfo = composer.createObjectTC(UpdateInfo_1.UpdateInfo);
    const pageInfo = composer.createObjectTC(PageInfo_1.PageInfo);
    if (subgraph) {
    ...
    const { interfaceTypes, scalarTypes, objectTypes, enumTypes, inputObjectTypes, directives, unionTypes, schemaExtensions, } = definitionNodes;
    const globalSchemaConfiguration = (0, schema_configuration_1.schemaConfigurationFromSchemaExtensions)(schemaExtensions);
    ...
    relationshipProperties.forEach((relationship) => {
        const authDirective = (relationship.directives || []).find((x) => ["auth", "authorization"].includes(x.name.value));
        if (authDirective) {
    ...
    nodes.forEach((node) => {
        ...
        const composeNode = composer.createObjectTC({
            ...
            directives: (0, to_compose_1.graphqlDirectivesToCompose)([...node.otherDirectives, ...node.propagatedDirectives]),
        ...
        composer.createObjectTC({
            ...
            directives: (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives),
        });
        ...
        const schemaConfigurationFlags = (0, schema_configuration_1.getSchemaConfigurationFlags)({
        ...
        if (schemaConfigurationFlags.read) {
            ...
            composer.Query.setFieldDirectives(rootTypeFieldNames.read, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
            composer.Query.addFields({
                [`${node.plural}Connection`]: (0, root_connection_1.rootConnectionResolver)({ node, composer }),
            });
            composer.Query.setFieldDirectives(`${node.plural}Connection`, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
        }
        if (schemaConfigurationFlags.aggregate) {
            composer.Query.addFields({
                [rootTypeFieldNames.aggregate]: (0, aggregate_1.aggregateResolver)({ node }),
            });
            composer.Query.setFieldDirectives(rootTypeFieldNames.aggregate, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
        }
        if (schemaConfigurationFlags.create) {
            composer.Mutation.addFields({
                [rootTypeFieldNames.create]: (0, create_1.createResolver)({ node }),
            });
            composer.Mutation.setFieldDirectives(rootTypeFieldNames.create, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
        }
        if (schemaConfigurationFlags.delete) {
            composer.Mutation.addFields({
                [rootTypeFieldNames.delete]: (0, delete_1.deleteResolver)({ node, composer }),
            });
            composer.Mutation.setFieldDirectives(rootTypeFieldNames.delete, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
        }
        if (schemaConfigurationFlags.update) {
            ...
            composer.Mutation.setFieldDirectives(rootTypeFieldNames.update, (0, to_compose_1.graphqlDirectivesToCompose)(node.propagatedDirectives));
        }
    });
    ...
    filteredInterfaceTypes.forEach((inter) => {
        ...
        composer.createInterfaceTC({
            ...
            fields: objectComposeFields,
            directives: (0, to_compose_1.graphqlDirectivesToCompose)((inter.directives || []).filter((x) => !["auth", "authorization", "exclude"].includes(x.name.value))),
        });
    });
    ...
    const emptyObjectsInterfaces = parsedDoc.definitions
        .filter((x) => (x.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION && !(0, is_root_type_1.isRootType)(x)) || x.kind === graphql_1.Kind.INTERFACE_TYPE_DEFINITION)
    ...
    const schemaExtensionsWithoutNeo4jDirectives = schemaExtensions.map((schemaExtension) => {
        return {
            ...
            directives: schemaExtension.directives?.filter((schemaDirective) => !["query", "mutation", "subscription"].includes(schemaDirective.name.value)),
        };
    });
    const seen = {};
    parsedDoc = {
