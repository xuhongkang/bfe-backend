...
const Neo4jGraphQLAuthorization_1 = require("../../classes/authorization/Neo4jGraphQLAuthorization");
...
const wrapResolver = ({ driver, config, nodes, relationships, schemaModel, plugins, dbInfo, authorization }) => (next) => async (root, args, context, info) => {
    ...
    if (!context.jwt) {
        ...
        if (authorization) {
            context.jwt = await new Neo4jGraphQLAuthorization_1.Neo4jGraphQLAuthorization(authorization).decode(req);
        }
        else {
    ...
    (0, add_measurement_field_1.addMeasurementField)(context, add_measurement_field_1.Measurement.wrapperTime, p2 - p1);
...
const wrapSubscription = (resolverArgs) => (next) => async (root, args, context, info) => {
    ...
    if (context?.jwt) {
        ...
        if (resolverArgs.authorization) {
            subscriptionContext.jwt = new Neo4jGraphQLAuthorization_1.Neo4jGraphQLAuthorization(resolverArgs.authorization).decodeBearerToken(contextParams.authorization);
        }
        else {
            // TODO: remove this else after migrating to new authorization constructor
