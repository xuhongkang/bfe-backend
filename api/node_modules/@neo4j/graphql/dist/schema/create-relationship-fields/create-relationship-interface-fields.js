...
function createRelationshipInterfaceFields({ nodes, rel, composeNode, schemaComposer, sourceName, hasNonGeneratedProperties, anyNonNullRelProperties, }) {
    ...
    const disconnectFieldInput = schemaComposer.getOrCreateITC(`${sourceName}${upperFieldName}DisconnectFieldInput`, (tc) => {
        tc.addFields({
    ...
    const updateConnectionFieldInput = schemaComposer.getOrCreateITC(`${sourceName}${upperFieldName}UpdateConnectionInput`, (tc) => {
        tc.addFields({
            ...(hasNonGeneratedProperties ? { edge: `${rel.properties}UpdateInput` } : {}),
            node: `${rel.typeMeta.name}UpdateInput`,
        });
    });
    if (nestedOperations.size !== 0 &&
        !(nestedOperations.size === 1 && nestedOperations.has(constants_1.RelationshipNestedOperationsOption.CONNECT_OR_CREATE))) {
        ...
        if (nestedOperations.has(constants_1.RelationshipNestedOperationsOption.DELETE)) {
        ...
        if (nestedOperations.has(constants_1.RelationshipNestedOperationsOption.DISCONNECT)) {
    ...
    if (nestedOperations.has(constants_1.RelationshipNestedOperationsOption.CONNECT) ||
        ...
        if (rel.settableOptions.onCreate && !(composeNode instanceof graphql_compose_1.InterfaceTypeComposer)) {
