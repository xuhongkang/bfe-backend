...
const add_measurement_field_1 = require("../utils/add-measurement-field");
async function translateUpdate({ node, context, }) {
        ...
        Object.entries(createInput).forEach((entry) => {
            ...
            refNodes.forEach((refNode) => {
                ...
                creates.forEach((create, index) => {
                    ...
                    if (!relationField.typeMeta.array) {
                        const singleCardinalityValidationTemplate = (nodeName) => `CALL apoc.util.validate(EXISTS((${varName})${inStr}[:${relationField.type}]${outStr}(:${nodeName})),'Relationship field "%s.%s" cannot have more than one node linked',["${relationField.connectionPrefix}","${relationField.fieldName}"])`;
    ...
    if (nodeProjection?.fieldsByTypeName) {
        ...
        if (projection.meta?.authValidatePredicates?.length) {
            projAuth = new cypher_builder_1.default.With("*").where(cypher_builder_1.default.apoc.util.validatePredicate(cypher_builder_1.default.not(cypher_builder_1.default.and(...projection.meta.authValidatePredicates)), constants_1.AUTH_FORBIDDEN_ERROR));
        }
    }
    ...
    (0, add_measurement_field_1.addMeasurementField)(context, add_measurement_field_1.Measurement.translationTime, p2 - p1);
    return result;
}
...
function generateUpdateReturnStatement(varName, projStr, subscriptionsEnabled) {
    let statements;
    if (varName && projStr) {
        statements = new cypher_builder_1.default.RawCypher((env) => `collect(DISTINCT ${varName} ${projStr.getCypher(env)}) AS data`);
    }
