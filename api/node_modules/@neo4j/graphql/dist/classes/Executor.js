...
const wrap_in_time_measurement_1 = require("../utils/wrap-in-time-measurement");
...
class Executor {
    constructor({ executionContext, auth, queryOptions, database, bookmarks, measureTime = false, }) {
    ...
    async execute(query, parameters, defaultAccessMode, info) {
        try {
    ...
    async sessionRun(query, parameters, defaultAccessMode, session, info) {
    ...
    async transactionRun(query, parameters, transaction) {
        ...
        const { result, time } = await (0, wrap_in_time_measurement_1.wrapInTimeMeasurement)(() => {
