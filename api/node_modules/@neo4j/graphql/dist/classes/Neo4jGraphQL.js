...
const make_schema_to_augment_1 = require("../schema/make-schema-to-augment");
...
class Neo4jGraphQL {
    ...
    neo4jValidateGraphQLDocument() {
        try {
            ...
            (0, validation_1.validateDocument)(initialDocument);
            const { document, typesExcludedFromGeneration } = (0, make_schema_to_augment_1.makeSchemaToAugment)(initialDocument);
            const { jwtPayload } = typesExcludedFromGeneration;
            const { typeDefs } = (0, schema_2.makeAugmentedSchema)(document, {
            ...
            (0, schema_validation_1.validateUserDefinition)({ userDocument: document, augmentedDocument: typeDefs, jwtPayload });
        }
        catch (error) {
    ...
    wrapResolvers(resolvers) {
        ...
        const mergedResolvers = (0, merge_1.mergeResolvers)([...(0, utils_2.asArray)(resolvers), ...(0, utils_2.asArray)(this.schemaDefinition.resolvers)]);
        return (0, resolvers_composition_1.composeResolvers)(mergedResolvers, resolversComposition);
    }
    wrapFederationResolvers(resolvers) {
        if (!this.schemaModel) {
            throw new Error("Schema Model is not defined");
        }
        const wrapResolverArgs = {
        ...
        const resolversComposition = {
            "Query.{_entities, _service}": [(0, wrapper_1.wrapResolver)(wrapResolverArgs)],
        };
        // Merge generated and custom resolvers
        const mergedResolvers = (0, merge_1.mergeResolvers)([...(0, utils_2.asArray)(resolvers)]);
        return (0, resolvers_composition_1.composeResolvers)(mergedResolvers, resolversComposition);
    }
    ...
    async generateSubgraphSchema() {
        ...
        const { validateTypeDefs, validateResolvers } = this.parseStartupValidationConfig();
        if (validateTypeDefs) {
            (0, validation_1.validateDocument)(initialDocument, directives, types);
        }
        const { document, typesExcludedFromGeneration } = (0, make_schema_to_augment_1.makeSchemaToAugment)(initialDocument);
        const { jwtPayload } = typesExcludedFromGeneration;
        const { nodes, relationships, typeDefs, resolvers } = (0, schema_2.makeAugmentedSchema)(document, {
        ...
        const subgraphResolvers = (0, utils_1.getResolversFromSchema)(schema);
        ...
        (0, schema_1.addResolversToSchema)({ schema, resolvers: wrappedSubgraphResolvers, updateResolversInPlace: true });
